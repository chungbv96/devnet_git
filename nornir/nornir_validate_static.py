...
def function_tasks(task):
    task.run(name="Show interface status", task=netmiko_send_command, command_string="show interface gi1/0/2 status")
    task.run(name="Create a loopback interface", task=netmiko_send_config, config_commands=["interface loop12", "description Created by nornir-scrapli"])
